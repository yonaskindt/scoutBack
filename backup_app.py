...
# Attempt to import statbotics for enhanced data
try:
    import statbotics
...
st.markdown("""
    ...
    .sb-status-box { padding: 8px; font-size: 11px; background: rgba(255, 255, 255, 0.03); border-radius: 5px; margin-bottom: 10px; border-left: 3px solid #4F8BF9; }
...
def load_frc_data():
    ...
    # --- FIXED STATBOTICS FETCH ---
    epa_map = {}
    sb_status = "❌ sb Library Missing"
    if sb:
        try:
            # 1. get_event returns a dictionary - used as a handshake
            event_info = sb.get_event("2026azfg")
            if isinstance(event_info, dict) and event_info.get("key"):
                # 2. Fetch team data. 
                # IMPORTANT: Specify limit > 0 and metric="team" to avoid "Invalid Query"
                teams_data = sb.get_team_events(event="2026azfg", limit=1000, metric="team")
                for entry in teams_data:
                    t_num = int(entry.get('team'))
                    # Field names vary by version: try 'epa_end', then 'epa'
                    epa_map[t_num] = entry.get('epa_end') or entry.get('epa') or 0
                sb_status = f"✅ Connected: {event_info.get('name', '2026azfg')}"
            else:
                sb_status = "⚠️ Event Key Not Found"
        except Exception as e:
            # Captures "Invalid Query" or connection errors
            error_msg = str(e).split(':')[-1].strip()
            sb_status = f"❌ API Error: {error_msg[:20]}"
            
    return data, pit, schema, ali, epa_map, sb_status

try:
    # Unpack 6 values including the status message
    df, pit_df, schema_df, alliance_df, epa_data, sb_status_msg = load_frc_data()
except Exception as e:
...
def get_team_stats(team_num):
    ...
    stat_dict = {"avg": 0, "count": 0, "climb_pref": "N/A", "last_note": "No data", "driver": 0, "died": 0, "def": 0, "warn": False, "auto": 0, "epa": 0}
    stat_dict['epa'] = epa_data.get(team_num, 0)
    
    if not t_data.empty:
        stat_dict["avg"] = t_data['Total Score'].mean()
        stat_dict["def"] = t_data['Defense_Num'].mean() * 100
        stat_dict["auto"] = t_data['Moved_Num'].mean() * 100
        stat_dict["count"] = len(t_data)
        if not t_data['Climbing'].dropna().empty:
            stat_dict["climb_pref"] = t_data['Climbing'].dropna().mode().iloc[0]
        if 'driver skill' in t_data:
            stat_dict["driver"] = t_data['driver skill'].mean()
        note_s = t_data.dropna(subset=['Comments'])
        if not note_s.empty:
            stat_dict['last_note'] = note_s.iloc[-1]['Comments']
        stat_dict['died'] = int(t_data['Died_Num'].sum() + t_data['Tipped_Num'].sum())
        if stat_dict['count'] > 0:
            stat_dict['warn'] = True if (stat_dict['died'] / stat_dict['count']) > 0.2 else False
...
def detailed_card(team_num, color_hex):
    ...
    st.markdown(f"""<div class="team-info-box-detailed" style="border-top-color: {color_hex};">
        ...
        <div class="stat-row">Avg: <b>{round(s['avg'], 1)}</b> <span>EPA: {s['epa']}</span></div>
    ...
    with f_m:
        if os.path.exists("field.png"):
            ...
            <div id="controls" style="display:flex; gap:10px; margin-bottom:5px;">
                ...
                <button onclick="clearCanvas()" style="flex:0.5; padding:8px; cursor:pointer; background:#E74C3C; color:white; border:none; border-radius:5px; font-weight:bold;">Clear</button>
            </div>
            <div id="field-container" style="position: relative; width: 100%; max-width: 1050px; margin: 0 auto; border: 2px solid #555; border-radius: 12px; overflow: hidden; touch-action: none; background-color: #000;">
                ...
                <div class="bot blue" style="right: 10%; top: 65%; z-index:100;">{blue_teams[2]}</div>
            </div>
            <style>
...
# --- 3. SIDEBAR NAVIGATION (Updated Header) ---
with st.sidebar:
    st.title("⚙️ HUB CONTROL")
    
    # Connection Status UI
    st.markdown(f"""
        <div style="padding: 10px; background: rgba(255,255,255,0.05); border-radius: 8px; border-left: 4px solid #4F8BF9; margin-bottom: 15px;">
            <div style="font-size: 10px; color: #BDC3C7; text-transform: uppercase; letter-spacing: 1px;">Statbotics Connectivity</div>
            <div style="font-size: 13px; font-weight: bold; color: white;">{sb_status_msg}</div>
        </div>
    """, unsafe_allow_html=True)
...
if view == "🏆 Playoffs":
    ...
    c1, c2 = st.columns(2)
    with c1:
        red_ali_name = st.selectbox("🔴 Red Alliance", a_names, index=a_names.index(st.session_state.playoff_red_ali))
        st.session_state.playoff_red_ali = red_ali_name
        r_row = alliance_df.iloc[a_names.index(red_ali_name)]
        ...
        if pd.notna(r_row['3e']):
            ...
            if swap_r:
                if st.session_state.playoff_red_out in r_roster:
                    sel_idx = r_roster.index(st.session_state.playoff_red_out)
                else:
                    sel_idx = 0
                out = st.selectbox("Red to sit out", r_roster, index=sel_idx)
                st.session_state.playoff_red_out = out
                r_roster = [int(r_row['3e']) if x == out else x for x in r_roster]
    with c2:
        blue_ali_name = st.selectbox("🔵 Blue Alliance", a_names, index=a_names.index(st.session_state.playoff_blue_ali))
        st.session_state.playoff_blue_ali = blue_ali_name
        b_row = alliance_df.iloc[a_names.index(blue_ali_name)]
        ...
        if pd.notna(b_row['3e']):
            ...
            if swap_b:
                if st.session_state.playoff_blue_out in b_roster:
                    sel_idx_b = b_roster.index(st.session_state.playoff_blue_out)
                else:
                    sel_idx_b = 0
                out_b = st.selectbox("Blue to sit out", b_roster, index=sel_idx_b)
...
# --- 7. VIEW: DEEP DIVE ---
    ...
    m[1].metric("EPA (Statbotics)", round(stats['epa'], 1))
...
# --- 8. VIEW: ALLIANCE SELECTION ---
    ...
    if st.session_state.active_team_selection:
        ...
        if st.button("Cancel Selection"):
            st.session_state.active_team_selection = None
            st.rerun()
    drafted = [st.session_state.alliances[i][k] for i in range(1, 9) for k in ['c','p1','p2'] if st.session_state.alliances[i][k] > 0]
    col_l, col_r = st.columns([1.2, 2.5])
    with col_l:
        st.subheader("📋 Draftable Teams")
        t_data = pd.DataFrame([{"t": t, **get_team_stats(t)} for t in sorted(set(df['Team Number'].unique())) if t > 0]).sort_values('avg', ascending=False)
        for _, tr in t_data.iterrows():
            t_num = int(tr['t'])
            is_taken = t_num in drafted
            is_active = st.session_state.active_team_selection == t_num
            summary = f"{t_num} — Avg: {round(tr['avg'],1)} | EPA: {round(tr['epa'],1)}"
            ...
            with grid[(i-1)%2]:
                with st.container(border=True):
                    ...
                    for l, k in [("Capt","c"),("Pick 1","p1"),("Pick 2","p2")]:
                        ...
                        btn_label = f"{l}: {curr}" if curr > 0 else f"{l}: empty"
                        if st.button(btn_label, key=f"a{i}{k}", use_container_width=True):
                            if st.session_state.active_team_selection:
                    ...
                    st.markdown(f"<div style='text-align:right; color:#4F8BF9;'>Σ Avg: {round(sum_v,1)}</div>", unsafe_allow_html=True)
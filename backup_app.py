...
def get_team_stats(team_num):
    ...
    if t_data.empty: return {"avg": 0, "count": 0, "climb_pref": "N/A", "last_note": "No data", "driver": 0, "died": 0, "def": 0, "warn": False, "auto": 0}
    ...
    note = t_data.dropna(subset=['Comments']).iloc[-1]['Comments'] if not t_data.dropna(subset=['Comments']).empty else "No notes"
...
def detailed_card(team_num, color_hex):
    ...
    warn = "⚠️ " if s['warn'] else ""
    st.markdown(f"""<div class="team-info-box-detailed" style="border-top-color: {color_hex};">
        <div class="stat-row"><b>{warn}{team_num}</b> <span>{s['count']} matches</span></div>
...
def render_field_draggable(red_teams, blue_teams, match_label, red_pred, blue_pred):
        ...
        if os.path.exists("field.png"):
            ...
            <script>
                ...
                bots.forEach(bot => {{
                    ...
                    const move = (e) => {{ if (!isDragging) return; const clientX = e.type.includes('touch') ? e.touches[0].clientX : e.clientX; const clientY = e.type.includes('touch') ? e.touches[0].clientY : e.clientY; const rect = container.getBoundingClientRect(); let x = ((clientX - rect.left - offsetX) / rect.width) * 100; let y = ((clientY - rect.top - offsetY) / rect.height) * 100; bot.style.left = Math.max(0, Math.min(96, x)) + '%'; bot.style.top = Math.max(0, Math.min(94, y)) + '%'; if(e.type.includes('touch')) e.preventDefault(); }};
...
# --- 4. VIEW: PLAYOFFS (Alliance Pull + Drag) ---
if view == "🏆 Playoffs":
    ...
    with c1:
        ...
        if pd.notna(r_row['3e']):
            ...
            if swap_r:
                try: out_idx = r_roster.index(st.session_state.playoff_red_out) if st.session_state.playoff_red_out in r_roster else 0
                except: out_idx = 0
    ...
    with c2:
        ...
        if pd.notna(b_row['3e']):
            ...
            if swap_b:
                try: out_b_idx = b_roster.index(st.session_state.playoff_blue_out) if st.session_state.playoff_blue_out in b_roster else 0
                except: out_b_idx = 0
    ...
    render_field_draggable(r_roster, b_roster, "PLAYOFF", r_score, b_score)
    bc = st.columns(3)
...
# --- 5. VIEW: FIELD MAP ---
    ...
    render_field_draggable(rt, bt, st.session_state.m_sel_val, sum(get_team_stats(t)['avg'] for t in rt), sum(get_team_stats(t)['avg'] for t in bt))
    b_cols = st.columns(3)
...
# --- 7. VIEW: DEEP DIVE ---
    ...
    m = st.columns(5)
    m[0].metric("Avg", round(stats['avg'],1)); m[1].metric("Auto Move", f"{round(stats['auto'])}%"); m[2].metric("Climb", stats['climb_pref']); m[3].metric("Def %", f"{round(stats['def'])}%"); m[4].metric("Samples", stats['count'])
    ...
    with r:
        ...
        if not p_dat.empty: st.subheader("🛠️ Pit"); st.dataframe(p_dat.T, use_container_width=True)
...
# --- 8. VIEW: ALLIANCE SELECTION ---
    ...
    with col_l:
        ...
        for _, tr in draft_sorted.iterrows():
            t_num = int(tr['team']); is_taken = t_num in drafted; is_active = st.session_state.active_team_selection == t_num
            summary = f"{t_num} — Avg: {round(tr['avg'],1)} | Def: {round(tr['def'])}%"
            with st.expander(f"{'➡️ ' if is_active else ''}{'~~' if is_taken else ''}{summary}"):
                if not is_taken:
                    if st.button(f"SELECT {t_num}", key=f"sel_{t_num}"): st.session_state.active_team_selection = t_num; st.rerun()
            ...
            with grid[(i-1)%2]:
                ...
                for l, k in [("Capt","c"),("Pick 1","p1"),("Pick 2","p2")]:
                    ...
                    if st.button(f"{l}: {curr if curr > 0 else 'empty'}", key=f"a{i}{k}", use_container_width=True):
                        if st.session_state.active_team_selection: st.session_state.alliances[i][k] = st.session_state.active_team_selection; st.session_state.active_team_selection = None; st.rerun()

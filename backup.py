...
# --- 1. DATA LOADING (EXCEL SHEET LOGIC) ---
@st.cache_data
def load_data(mode):
    ...
    try:
        ...
        for d in [schema, data, ali]: 
            ...
            if mode == "FTC":
                # Only drop columns that are completely empty to preserve robot data
                d.dropna(how='all', axis=1, inplace=True)
                # Filter out those "Unnamed" artifacts
                cols_to_keep = [c for c in d.columns if "Unnamed" not in str(c)]
                d = d[cols_to_keep]
...
def render_field_interactive(red_teams, blue_teams, match_label, red_pred, blue_pred):
    ...
    aspect_ratio = "45%" if st.session_state.comp_mode == "FRC" else "100%"
    max_width = "100%" if st.session_state.comp_mode == "FRC" else "550px"
    ...
    with f_m:
        ...
        html_content = f"""
        <div id="controls" style="display:flex; gap:10px; margin:0 auto; max-width:{max_width}; padding-bottom:2px;">
        ...
        <div id="field-container" style="position: relative; width: 100%; max-width:{max_width}; margin: 0 auto; padding-bottom: {aspect_ratio}; border: 2px solid #555; border-radius: 8px; overflow: hidden; background-color: #000; touch-action: none; display:block;">
            <img src="data:image/png;base64,{img_b64}" style="position:absolute; top:0; left:0; width:100%; height:100%; object-fit: contain; pointer-events: none;">
        ...
        <script>
            ...
            bots.forEach(bot => {{
                ...
                bot.addEventListener('touchstart', (e) => {{ if(mode==='move') isDragging = true; e.preventDefault(); }}, {{passive:false}});
...
if view == "🏆 Playoffs":
    ...
    with c1:
        ...
        r_roster = [int(x) for x in r_row[['C','1e','2e'] if '2e' in alliance_df.columns else ['C','1e']].dropna()]
    with c2:
        b_choice = st.selectbox("🔵 Blue Alliance", a_names, index=min(1, len(a_names)-1))
        b_row = alliance_df.iloc[a_names.index(b_choice)]
        b_roster = [int(x) for x in b_row[['C','1e','2e'] if '2e' in alliance_df.columns else ['C','1e']].dropna()]

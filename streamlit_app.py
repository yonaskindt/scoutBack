...
# Playoff Persistence
if 'playoff_red_ali' not in st.session_state: st.session_state.playoff_red_ali = "Alliance 1"
if 'playoff_blue_ali' not in st.session_state: st.session_state.playoff_blue_ali = "Alliance 2"
...
def load_frc_data():
    ...
    def clean_bool(val):
        if str(val).lower() in ['true', '1', '1.0', 'yes', 'y']: return 1
        return 0
...
def get_team_stats(team_num):
    ...
    stat_dict = {"avg": 0, "count": 0, "climb_pref": "N/A", "last_note": "No data", "driver": 0, "died": 0, "def": 0, "warn": False, "auto": 0}
    if not t_data.empty:
...
with st.sidebar:
    ...
    if view in ["🗺️ Field Map", "📊 Overview"]:
        ...
        try: m_idx = m_list.index(st.session_state.m_sel_val)
        except: m_idx = 0
...
if view == "🏆 Playoffs":
    ...
    with c1:
        ...
        if pd.notna(r_row['3e']):
            ...
            if swap_r:
                sel_idx = r_roster.index(st.session_state.playoff_red_out) if st.session_state.playoff_red_out in r_roster else 0
    ...
    with c2:
        ...
        if pd.notna(b_row['3e']):
            ...
            if swap_b:
                sel_idx_b = b_roster.index(st.session_state.playoff_blue_out) if st.session_state.playoff_blue_out in b_roster else 0
...
# --- 5. VIEW: FIELD MAP ---
    ...
    for i, t in enumerate(rt):
        with rc_cols[i]: detailed_card(t, "#FF4B4B")
    ...
    for i, t in enumerate(bt):
        with bc_cols[i]: detailed_card(t, "#1F77B4")
...
# --- 7. VIEW: PER TEAM ---
    ...
    # Dynamic Team Name Lookup
    p_row = pit_df[pit_df['team_number'] == sel_t]
    t_name = ""
    for col in ['team_name', 'Team Name', 'name', 'Name']:
    ...
    m[2].metric("Def %", f"{round(stats['def'])}%")
...
# --- 8. VIEW: ALLIANCE SELECTION ---
    ...
    if st.session_state.active_team_selection:
        ...
        if st.button("Cancel Selection"): st.session_state.active_team_selection = None; st.rerun()
    ...
    with col_l:
        ...
        for _, tr in t_data.iterrows():
            t_num = int(tr['t']); is_taken = t_num in drafted; is_active = st.session_state.active_team_selection == t_num
            summary = f"{t_num} — Avg: {round(tr['avg'],1)}"
            with st.expander(f"{'➡️ ' if is_active else ''}{'~~' if is_taken else ''}{summary}"):
                if not is_taken and st.button(f"SELECT {t_num}", key=f"sel_{t_num}"):
                ...
                with st.container(border=True):
                    ...
                    for l, k in [("Capt","c"),("Pick 1","p1"),("Pick 2","p2")]:
                        ...
                        if st.button(btn_label, key=f"a{i}{k}", use_container_width=True):
                            ...
                            else: st.session_state.alliances[i][k] = 0; st.rerun()
